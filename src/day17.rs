...
*/
    ...
    let mut grid: Vec<Vec<char>> = lines.iter().map(|s| s.chars().collect()).collect();
    // store direction like 'l', 'r', 'u', 'd'
    let mut visited_to_total: HashMap<(usize, usize, char, usize, usize), usize> = HashMap::new();
    // priority is total_path_sum
    let mut queue: PriorityQueue<(usize, usize, char, usize, usize, usize), (i32, i32)> = PriorityQueue::new();
    // we start at top left corner, but don't have any direction
    queue.push((1, 0, 'r', 2, 0, 0), (0, 0));
    queue.push((0, 1, 'd', 2, 0, 0), (0, 0));
    // we end at bottom right corner
    let last_x = grid.len() - 1;
    let last_y = grid[0].len() - 1;
    let mut res: i32 = -1;
    while !queue.is_empty() {
        let ((x, y, direction, count, straight_count, total_path_sum), (priority, manhattan)) = queue.pop().unwrap();
        //println!("x: {}, y: {}, direction: {}, distance: {}, total_path_sum: {}, cell {}",
        //         x, y, direction, count, total_path_sum, grid[y][x]);

        let mh = (last_x as i32 - x as i32).abs() + (last_y as i32 - y as i32).abs();
        let prev_total = visited_to_total.entry((x, y, direction, count, straight_count)).or_insert(0);
        // check if we visited this cell
        if *prev_total > 0 && *prev_total <= total_path_sum {
            continue;
        }
        visited_to_total.insert((x, y, direction, count, straight_count), total_path_sum);
        ...
        let priority: i32 = -(total as i32);
        // let's match (direction)
        let can_go_straight = count < 10;
        let can_turn = straight_count == 0;
        let straight_count_new = if straight_count == 0 { 0 } else { straight_count - 1 };
        let st = 3;
